...
beowulfBroadcast.send = function beowulfBroadcast$send(tx, privKeys, callback) {
  const resultP = beowulfBroadcast
...
beowulfBroadcast.create = function beowulfBroadcast$create(
  tx,
  privKeys,
  callback,
) {
...
beowulfBroadcast.broadcast = function beowulfBroadcast$broadcast(
  signedTransaction,
  callback,
) {
  const resultP = beowulfApi
    .broadcastTransactionSynchronousAsync(signedTransaction)
    .then(result => {
...
beowulfBroadcast._prepareTransaction = function beowulfBroadcast$_prepareTransaction(
  tx,
) {
  ...
  return propertiesP.then(properties => {
    ...
    return beowulfApi
      .getBlockAsync(properties.last_irreversible_block_num)
      .then(block => {
        ...
        return Object.assign(
          {
            ...
            expiration: new Date(chainDate.getTime() + 3600 * 1000),
          },
          tx,
        );
      });
...
operations.forEach(operation => {
  ...
  beowulfBroadcast[
    `${operationName}With`
  ] = function beowulfBroadcast$specializedSendWith(
    wif,
    options,
    data_extension,
    callback,
  ...
  beowulfBroadcast[operationName] = function beowulfBroadcast$specializedSend(
    wif,
    ...args
  ) {
    ...
    return beowulfBroadcast[`${operationName}With`](
      wif,
      options,
      data_extension,
      callback,
  ...
  beowulfBroadcast[`${operationName}Multisig`] = function(wif, ...args) {
    debug(`Parsing operation "${operationName}" with`, { args });
    const options = operationParams.reduce((memo, param, i) => {
      memo[param] = args[i]; // eslint-disable-line no-param-reassign
      return memo;
    ...
    return beowulfBroadcast[`${operationName}MultisigWith`](
      wif,
      options,
      callback,
    );
  ...
  beowulfBroadcast[`${operationName}MultisigWith`] = function(
    wif,
    options,
    callback,
  ) {
